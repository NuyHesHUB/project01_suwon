...
$(document).ready(function(){
            ...
            , viewSet : function(populationDataSet){
                ...
                try {
                    if(populationDataSet != null && populationDataSet != '') {
                        ...
                        htmlTemplate = htmlTemplate.replace(/#local#/ig, $this.format(populationData.local));
                        htmlTemplate = htmlTemplate.replace(/#foreign#/ig, $this.format(populationData.foreign));
                        htmlTemplate = htmlTemplate.replace(/#house#/ig, $this.format(populationData.house));
                        htmlTemplate = htmlTemplate.replace(/#year#/ig, populationData.year);
                        htmlTemplate = htmlTemplate.replace(/#month#/ig, populationData.month);
                        htmlTemplate = htmlTemplate.replace(/#day#/ig, populationData.day);
            ...
            , init : function(){
                ...
                if($.cookie('swpsd') != null && $.cookie('swpsd') != '') {
                    ...
                    $.ajax({
                        type : 'get'
                        , dataType : 'xml'
                        , url : '/stat/stats/INC_DataView.do'
                        , data : {'categorySeqNo' : '14', 'rtn' : 'xml'}
                        , success : function(xml){
                            try {
                                if(xml != null){
                                    if($(xml).find('list').find('item').length > 0){
                                        var totalEle = $(xml).find('list').find('item[id="total_sum"]');
                                        var totalCnt = totalEle.find('col4').text();
                                        var localCnt = totalEle.find('col5').text();
                                        var foreignCnt = totalEle.find('col10').text();
                                        var houseCnt = totalEle.find('col3').text();
                                        var baseYear = $(xml).find('list').attr('baseyear');
                                        var baseMonth = $(xml).find('list').attr('basemonth');
                                        var baseDay = $(xml).find('list').attr('baseday');
                                        // 쿠키에 담을 데이터를 세팅
                                        populationDataSet = '{';
                                        populationDataSet += '"total": "' + $.trim(totalCnt) + '",';
                                        populationDataSet += '"local": "' + $.trim(localCnt) + '",';
                                        populationDataSet += '"foreign": "' + $.trim(foreignCnt) + '",';
                                        populationDataSet += '"house": "' + $.trim(houseCnt) + '",';
                                        populationDataSet += '"year": "' + $.trim(baseYear) + '",';
                                        populationDataSet += '"month": "' + $.trim(baseMonth) + '",';
                                        populationDataSet += '"day": "' + $.trim(baseDay) + '"';
                                        populationDataSet += '}';
    ...
    $.ajax({
        ...
        , type : 'GET'
        , cache : false
        , data : {target:'rankings', max_count:'10'}
        , dataType : 'json'
